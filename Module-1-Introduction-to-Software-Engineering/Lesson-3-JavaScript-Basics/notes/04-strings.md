...
- Understand the string data type and type coercion in JavaScript.
- Explore string concatenation and the impact of type coercion with the + operator.
- Understand zero-indexing, bracket notation, and the concept of string immutability.
- Implement ES6 template literals and essential string methods (indexOf(), slice(), toUpperCase(), toLowerCase(), concat(), split()) for practical applications.
...
If you guessed `5`, you'd be wrong. Yup. You're actually going to get `"32"`. This is because the `+` operator, when used in between two strings, combines them! Like I mentioned above... Again, this is a characteristic of the `String` data type, and this phenomenon is called `concatenation` - which is a fancy way of saying the two strings get combined.
...
- 2. Use the **_`Unary Plus`_** operator (`+`)
...
> **_Note:_** you don't have to follow the format of these examples exactly. Once you begin to see the smaller pieces to what's happening above, you'll be able to write JavaScript however you want, as long as you are following the syntax rules in JavaScript. **_You need to experiment!!!!_**
...
**_Practice: Lets take a moment to write this into our own notes and test it. Change the values within the variables and see what the output is._**
...
console.log(
  "Aren't we suppose to wait 30 minutes after eating before we jump into the pool?"
);
// Or we can utilize a different type of quotes to avoid conflict
// Double Quotes
console.log(
  "Aren't we suppose to wait 30 minutes after eating before we jump into the pool?"
);
...
console.log(
  "I am hungry and could go for ".concat(
    food,
    " with ",
    toppings,
    " on top and ",
